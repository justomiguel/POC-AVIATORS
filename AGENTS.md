...
Este repo es **Google Apps Script** con **clasp** (`gas/`, despliegue a un proyecto Script). **No** hacer `clasp push` ni despliegue a prod sin consentimiento explícito del usuario (ver **`.cursor/rules/no-production-push-without-consent.mdc`**). Las reglas detalladas están en **`.cursor/rules/*.mdc`** (stack, dependencias, i18n, **límite ~60 KB de `google.script.run`**, manifiesto, convención de commits sin marcas de IDE). Contexto de producto y diseño: **`PRODUCT.md`**, **`DESIGN.md`**. Convención de **icono en cada botón** de la UI: **`gas-ui-buttons-icons.mdc`**. Convención de **secciones** (cabecera + párrafo explicativo): **`gas-ui-sections-structure.mdc`**. **Errores al usuario** (modal centralizado + detalle técnico): **`gas-ui-error-modal.mdc`**. Tras tocar Tailwind o clases en HTML de `gas/`, recompilar CSS: **`gas-tailwind-compile.mdc`**.
